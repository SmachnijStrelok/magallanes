 ...
 * Usage :
 *   pre-deploy:
 *     - magento/link-shared-files: {link:/path/to/symlink, target: /path/to/target}
 *   on-deploy:
 *     - magento/link-shared-files: {link:/path/to/symlink, target: /path/to/target}
 *
...
class LinkSharedFilesTask extends AbstractTask implements IsReleaseAware
{
    ...
    public function run()
    {
        $linkedEntities = array_merge(
            $this->getConfig('extras', 'shared', 'linked_files', array()),
            $this->getConfig('extras', 'shared', 'linked_folders', array())
        );
